...
// CSS Modules import. 객체로 받아서 styles.클래스명 으로 사용
import styles from './rsp.module.css';

// 직접 만든 Box 컴포넌트 import (component 폴더 안)
import Box from './components/Box';

// body에 페이지 전용 클래스 부여 → 다른 페이지 CSS와 충돌 방지
...
const choice = {
  ...
  paper:    { name: "Paper",    emoji: "🖐️" }
};

// ===================================================================
// Rsp 컴포넌트 정의 - 화살표 함수가 아니라 일반 function 선언 사용
// 컴포넌트 이름은 반드시 대문자로 시작 (PascalCase) - React 규칙
// ===================================================================
function RspPage() {
  useEffect(() => {
    document.body.className = 'page-rsp';
  ...
  const randomChoice = () => {
    ...
    return choice[keys[Math.floor(Math.random() * keys.length)]];
  };
  ...
  return (
    <div className={styles.app}>
      ...
      <div className={styles.scoreboard}>
        <div className={`${styles['score-item']} ${styles['win-score']}`}>
          <span className={styles['score-label']}>WIN</span>
          <span className={styles['score-value']}>{String(score.win).padStart(2, '0')}</span>
        </div>
        <div className={`${styles['score-item']} ${styles['draw-score']}`}>
          <span className={styles['score-label']}>DRAW</span>
          <span className={styles['score-value']}>{String(score.draw).padStart(2, '0')}</span>
        </div>
        <div className={`${styles['score-item']} ${styles['lose-score']}`}>
          <span className={styles['score-label']}>LOSE</span>
          <span className={styles['score-value']}>{String(score.lose).padStart(2, '0')}</span>
        </div>
      </div>
      ...
      <div className={styles['result-banner']}>
        {isRolling && <span className={`${styles['result-text']} ${styles.rolling}`}>READY...</span>}
        {!isRolling && result === "WIN"  && <span className={`${styles['result-text']} ${styles.win}`}>★ YOU WIN ★</span>}
        {!isRolling && result === "LOSE" && <span className={`${styles['result-text']} ${styles.lose}`}>✗ YOU LOSE ✗</span>}
        {!isRolling && result === "DRAW" && <span className={`${styles['result-text']} ${styles.draw}`}>◆ DRAW ◆</span>}
      </div>
      ...
      <div className={styles.controls}>
        <button className={styles['game-btn']} onClick={() => play("scissors")} disabled={isRolling}>
          <span className={styles['btn-emoji']}>✌️</span>
          <span className={styles['btn-label']}>가위</span>
        </button>
        <button className={styles['game-btn']} onClick={() => play("rock")} disabled={isRolling}>
          <span className={styles['btn-emoji']}>✊</span>
          <span className={styles['btn-label']}>바위</span>
        </button>
        <button className={styles['game-btn']} onClick={() => play("paper")} disabled={isRolling}>
          <span className={styles['btn-emoji']}>🖐️</span>
          <span className={styles['btn-label']}>보</span>
        </button>
      </div>

      {/* 리셋 버튼 */}
      <button className={styles['reset-btn']} onClick={reset} disabled={isRolling}>
        ↺ RESET
      </button>

      <footer className={styles.footer}>© 2026 PIXEL ARCADE BY ZENO</footer>
    </div>
  );
}
